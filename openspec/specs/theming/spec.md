...
Defines light/dark theme selection for the frontend: a dark colour palette applied by overriding the design-system CSS variables, a toggle in the profile modal, per-device persistence, and an OS-preference default. Theming is a per-device display preference (stored in `localStorage`), independent of the global, admin-gated settings store.
