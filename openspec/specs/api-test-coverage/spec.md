# api-test-coverage Specification

## Purpose

Define the automated-test coverage the Go API must maintain for its critical paths — authentication, resource validation, CRUD round-trips — and the contract for database-dependent tests, so that regressions in these areas fail the build and the suite runs cleanly with or without a database.
